...
from nemo.collections.asr.parts.preprocessing.features_from_np import WaveformFeaturizer
...
class TTSDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            n_segments = int(self.segment_max_duration * sample["audio"]["sampling_rate"])
            if n_segments < sample["audio"]["array"].size:
            ...
            if self.pitch_augment:
                audio_shifted = self.pitch_shift(
                    features.cpu().detach().numpy(), self.sample_rate, sample["audio"]["path"]
                )
        ...
        if "text_tokens" in sample:
            ...
            tokenized = self.text_tokenizer(sample["ipa"])
        ...
        if LogMel in self.sup_data_types_set:
            ...
            if mel_path is not None and Path(mel_path).exists():
                ...
                mel_path = self.log_mel_folder / f"{sample['audio']['path']}.pt"
        ...
        for i, voiced_item in enumerate([Pitch, Voiced_mask, P_voiced]):
            if voiced_item in self.sup_data_types_set:
                ...
                voiced_filepath = voiced_folder / f"{sample['audio']['path']}.pt"
        ...
        if Energy in self.sup_data_types_set:
            energy_path = self.energy_folder / f"{sample['audio']['path']}.pt"
